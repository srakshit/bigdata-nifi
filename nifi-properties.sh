...
sed -i -e "s|^nifi.cluster.is.node=.*$|nifi.cluster.is.node=$NIFI_CLUSTER_MODE|" $NIFI_HOME/conf/nifi.properties
sed -i -e "s|^nifi.cluster.flow.election.max.candidates=.*$|nifi.cluster.flow.election.max.candidates=$NIFI_ELECTION_MAX_CANDIDATES|" $NIFI_HOME/conf/nifi.properties
